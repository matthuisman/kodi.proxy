...
def run(url=None, module='default', need_quote=False):
    ...
    addon_path = os.path.join(addons_dir, addon_id)
    log("Run split.fragment {} split.query {}".format(split.fragment, split.query))
    if need_quote:
        fragment = quote_plus(split.fragment, ':&=') if split.fragment else ''
        query = quote_plus(split.query, ':&=') if split.query else ''
    else:
        fragment = split.fragment if split.fragment else ''
        query = split.query if split.query else ''
    log("Run fragment {} query {}".format(fragment, query))
...
def output_shell(listitem):
    ...
    output_json_dump({"url": listitem.getPath(), "label": listitem.getLabel(), "property": listitem._data['property']})
